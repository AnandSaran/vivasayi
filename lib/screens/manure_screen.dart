...
class ManureScreen extends StatefulWidget {
  final HomeNavigationItemIdEnum id;

  const ManureScreen({Key? key, required this.id}) : super(key: key);

  @override
  _ManureScreenState createState() => _ManureScreenState();
}

class _ManureScreenState extends State<ManureScreen> {
  @override
  ...
  Widget _buildShopFilterView() {
    return BlocBuilder<ManureShopAddressBloc, ShopAddressState>(
  ...
  _buildShopListView() {
    return BlocBuilder<ManureShopBloc, ShopState>(builder: (context, state) {
  ...
  void loadShop(Location location) {
    context.read<ManureShopBloc>().add(LoadShop(location));
    context.read<ManureShopAddressBloc>().add(LoadShopAddress(location));
  }
