...
import 'package:vivasayi/bloc/shop_address/agri_product_shop_address_bloc.dart';
...
class AgriProductsScreen extends StatefulWidget {
  final HomeNavigationItemIdEnum id;

  const AgriProductsScreen({Key? key, required this.id}) : super(key: key);

  @override
  _AgriProductsScreenState createState() => _AgriProductsScreenState();
}

class _AgriProductsScreenState extends State<AgriProductsScreen> {
  @override
  ...
  Widget _buildShopFilterView() {
    return BlocBuilder<AgriProductShopAddressBloc, ShopAddressState>(
  ...
  _buildShopListView() {
    return BlocBuilder<AgriProductShopBloc, ShopState>(
        builder: (context, state) {
  ...
  void loadShop(Location location) {
    context.read<AgriProductShopBloc>().add(LoadShop(location));
    context.read<AgriProductShopAddressBloc>().add(LoadShopAddress(location));
  }
